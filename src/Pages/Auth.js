...
const Auth = () => {
    ...
    return (
        <div className="min-h-screen bg-primary-500 flex items-center justify-center p-4 md:p-3 lg:p-5 3xl:p-10overflow-auto">


            {/* Main Container */}
            <div className="w-full  my-auto max-w-[92%] sm:max-w-[600px] md:max-w-[750px] lg:max-w-[1100px] xl:max-w-[1100px] 3xl:max-w-[1500px] bg-white rounded-[30px] md:rounded-[40px] lg:rounded-[50px] p-6 sm:p-8 md:p-6 lg:p-8 xl:p-12 3xl:p-20 shadow-2xl overflow-autorelative">


                {/* Language Toggle Button - Inside Container */}
                <button
                    onClick={toggleLanguage}
                    className="absolute top-4 md:top-6 lg:top-8  ltr:right-4 rtl:left-4 md:ltr:right-6 md:rtl:left-6 lg:ltr:right-8 lg:rtl:left-8 z-50 bg-primary-50 hover:bg-primary-600 active:bg-primary-700 text-white rounded-full w-10 h-10 md:w-12 md:h-12 flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-200 border-2 border-white"
                    aria-label="Toggle Language"
                    title={currentLang === 'ar' ? 'Switch to English' : 'التبديل إلى العربية'}>
                    <span className="font-Cairo font-bold text-sm md:text-base"> {currentLang === 'ar' ? 'EN' : 'ع'} </span>
                </button>
                ...
                <div className="w-full grid  grid-cols-1 lg:grid-cols-2  gap-6 md:gap-6 lg:gap-8 xl:gap-12 place-items-center">
                    ...
                    <div className="order-2 lg:order-1 w-full max-w-md lg:max-w-full flex flex-col  justify-center items-center gap-4 md:gap-4 lg:gap-4 xl:gap-6 py-4 ">
                        ...
                        <div className=" w-full  flex justify-center mb-2 md:mb-3 lg:mb-2">
                            <img src={EgyptAirLogo} alt="Logo"
                                className=" w-[200px] sm:w-[280px] md:w-[350px] lg:w-[320px] xl:w-[400px] 3xl:w-[490px] h-auto object-contain"/>
                        </div>
